...
alterState(state => {
  ...
  state.data = {
    surveys: [
      ...
      { id: 'ad7S4hPBN7qM4Ac3mpEdwZ', tag: 'bns_price_2019' },
...
each(dataPath('surveys[*]'), state =>
  get(state.data.url, {}, state => {
    const theseSubmissions = state.data.results.map(submission => {
      return {
    ...
    console.log(`Fetched ${state.data.count} submissions from ${state.data.url}.`);
    //Once we fetch the data, we want to post each individual Kobo survey
    //back to the OpenFn inbox to run through the jobs =========================
    state.submissions.push(theseSubmissions);
    return state;
    // return each(
...
alterState(state => {
  ...
  console.log(`Gathered ${state.submissions.flat().length} submissions.`);
  return state;
});