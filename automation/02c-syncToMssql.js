...
each('$.lookupTables[*]', state => {
  ...
  console.log(`extcode: ${state.data.select_from_list_name}ExtCode`);
  for (choice in choiceDictionary) {
    if (name.includes(choice)) {
      for (value of choiceDictionary[choice]) {
        let obj = {};
        obj[`${prefixes}${state.data.select_from_list_name}ExtCode`] = value;
        obj[`${prefixes}${state.data.select_from_list_name}Name`] = value;
        mapping.push(obj);
      }
