...
fn(async state => {
 const mapping = {
  ...
  TT_ProportionID_PropSettlement: await findValue({uuid: 'tt_landproportionid', relation: 'TT_LandProportion', where: { TT_proportionExtCode: dataValue('prop_settlement') }})(state),
  TT_ProportionID_PropWetland: await findValue({uuid: 'tt_landproportionid', relation: 'TT_LandProportion', where: { TT_proportionExtCode: dataValue('prop_wetland') }})(state),
  TT_ProportionID_PropNativeVeg: await findValue({uuid: 'tt_landproportionid', relation: 'TT_LandProportion', where: { TT_proportionExtCode: dataValue('prop_native_veg') }})(state),
  TT_ProportionID_PropFallowLand: await findValue({uuid: 'tt_landproportionid', relation: 'TT_LandProportion', where: { TT_proportionExtCode: dataValue('prop_fallow_land') }})(state),
  TT_ProportionID_PropGrazingLand: await findValue({uuid: 'tt_landproportionid', relation: 'TT_LandProportion', where: { TT_proportionExtCode: dataValue('prop_grazing_land') }})(state),
  TT_ProportionID_PropCropLand: await findValue({uuid: 'tt_landproportionid', relation: 'TT_LandProportion', where: { TT_proportionExtCode: dataValue('prop_crop_land') }})(state),
  TotalAreaFarm: dataValue('total_area_farm'),
  WhereAnimalsShifted: x['where_animals_shifted'],
  TT_ActionID_WhatHappenGrazingAnimals: await findValue({uuid: 'tt_animalactionid', relation: 'TT_AnimalAction', where: { TT_actionExtCode: dataValue('what_happen_grazing_animals') }})(state),
  WhyNotContinuedUse: dataValue('why_not_continued_use'),
  TT_LengthID_FallowPeriod: await findValue({uuid: 'tt_fallowperiodid', relation: 'TT_FallowPeriod', where: { TT_lengthExtCode: dataValue('fallow_period') }})(state),
  TypeTreeCrops: x['type_tree_crops'],
  TT_CountID_NbAnimalsOnParcel: await findValue({uuid: 'tt_animalsonparcelid', relation: 'TT_AnimalsOnParcel', where: { TT_countExtCode: dataValue('nb_animals_on_parcel') }})(state),
  TypeGrazingAnimals: x['type_grazing_animals'],
  MainCrop: dataValue('main_crop'),
  TT_LandID_CurrentLanduse: await findValue({uuid: 'tt_landusetypeid', relation: 'TT_LandUseType', where: { TT_landExtCode: dataValue('current_landuse') }})(state),
  ...
  TT_InterventionsID_ProposedIntervention: await findValue({uuid: 'tt_interventiontypeid', relation: 'TT_InterventionType', where: { TT_interventionsExtCode: dataValue('proposed_intervention') }})(state),
  ...
  TT_CountryID_Country: await findValue({uuid: 'tt_regionid', relation: 'TT_Region', where: { TT_countryExtCode: dataValue('country') }})(state),
