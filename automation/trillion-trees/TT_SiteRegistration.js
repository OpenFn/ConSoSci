...
each(dataPath('type_grazing_animals[*]'), each(dataPath('undefined[*]'), fn(async state => {
 ...
 return upsertMany('TT_Animal', 'GeneratedUuid', () => mapping, {setNull: ["''", "'undefined'"]})(state); 
}) 
))
...
fn(async state => {
 ...
 return upsert('TT_KoboDataset', 'DatasetId', mapping, {setNull: ["''", "'undefined'"]})(state); 
}) 
fn(async state => {
 const mapping = {
  ...
  OnlyFarmOwned: await findValue({uuid: 'tt_yesnoid', relation: 'TT_yesno', where: { TT_yesnoExtCode: dataValue('only_farm_owned') }})(state),
  SketchMapPhoto: dataValue('photo_sketch_map'),
  TT_LandProportionID_Settlement: await findValue({uuid: 'tt_landproportionid', relation: 'TT_LandProportion', where: { TT_LandProportionExtCode: dataValue('prop_settlement') }})(state),
  TT_LandProportionID_Wetland: await findValue({uuid: 'tt_landproportionid', relation: 'TT_LandProportion', where: { TT_LandProportionExtCode: dataValue('prop_wetland') }})(state),
  TT_LandProportionID_NativeVegetation: await findValue({uuid: 'tt_landproportionid', relation: 'TT_LandProportion', where: { TT_LandProportionExtCode: dataValue('prop_native_veg') }})(state),
  TT_LandProportionID_FallowLand: await findValue({uuid: 'tt_landproportionid', relation: 'TT_LandProportion', where: { TT_LandProportionExtCode: dataValue('prop_fallow_land') }})(state),
  TT_LandProportionID_GrazingLand: await findValue({uuid: 'tt_landproportionid', relation: 'TT_LandProportion', where: { TT_LandProportionExtCode: dataValue('prop_grazing_land') }})(state),
  TT_LandProportionID_CropLand: await findValue({uuid: 'tt_landproportionid', relation: 'TT_LandProportion', where: { TT_LandProportionExtCode: dataValue('prop_crop_land') }})(state),
  FarmArea: dataValue('total_area_farm'),
  TT_AnimalActionShiftID: x['where_animals_shifted'],
  TT_AnimalActionID: await findValue({uuid: 'tt_animalactionid', relation: 'TT_AnimalAction', where: { TT_AnimalActionExtCode: dataValue('what_happen_grazing_animals') }})(state),
  TT_LandNotRequiredTypeID: await findValue({uuid: 'tt_landnotrequiredtypeid', relation: 'TT_LandNotRequiredType', where: { TT_LandNotRequiredTypeExtCode: dataValue('why_not_continued_use')}})(state),
  TT_FallowPeriodID: await findValue({uuid: 'tt_fallowperiodid', relation: 'TT_FallowPeriod', where: { TT_FallowPeriodExtCode: dataValue('fallow_period') }})(state),
  TT_TreeTypeID: x['type_tree_crops'],
  TT_AnimalsOnParcelID: await findValue({uuid: 'tt_animalsonparcelid', relation: 'TT_AnimalsOnParcel', where: { TT_AnimalsOnParcelExtCode: dataValue('nb_animals_on_parcel') }})(state),
  TT_AnimalID: x['type_grazing_animals'],
  MainCrop: dataValue('main_crop'),
  TT_LandUseID: await findValue({uuid: 'tt_landusetypeid', relation: 'TT_LandUseType', where: { TT_LandUseTypeExtCode: dataValue('current_landuse') }})(state),
  ...
  ProposedInterventionDescription: dataValue('please_describe_the_roposed_intervention'),
  TT_InterventionTypeID: await findValue({uuid: 'tt_interventiontypeid', relation: 'TT_InterventionType', where: { TT_InterventionTypeExtCode: dataValue('proposed_intervention') }})(state),
  ...
  TT_LandTitleTypeID: await findValue({uuid: 'tt_landtitletypeid', relation: 'TT_LandTitleType', where: { TT_LandTitleTypeExtCode: dataValue('type_land_title') }})(state),
  ...
  TT_RegionID: await findValue({uuid: 'tt_regionid', relation: 'TT_Region', where: { TT_RegionExtCode: dataValue('country') }})(state),
